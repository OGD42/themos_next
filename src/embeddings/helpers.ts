...
export function extractTsDocNode(nodeToFind: string, definition: any) {
  ...
  while (i < nodePath.length) {
    ...
    if (currentNode == null) {
      console.log(
        `Cant find ${nodePath[i]} in ${previousNode.children.map((x) => "\n" + x.name)}`
      );
      break;
    }
    i++;
...
export function generateParameters(tsDefinition: any) {
  ...
  const paramDefinitions: TsDoc.TypeDefinition[] =
    functionDeclaration.signatures.at(-1).parameters;
...
function recurseThroughParams(paramDefinition: any) {
  ...
  if (param.type?.type === "literal") {
    ...
    // primitive types
    if (
      !["string", "number", "boolean", "object", "unknown"].includes(
        param.type?.name
      )
    ) {
      // skip for now
...
function extractParamTypeAsString(paramDefinition) {
  if (paramDefinition.type?.name) {
    ...
    if (
      paramDefinition.type.types.every(({ type }) =>
        ["literal", "intrinsic"].includes(type)
      )
    ) {
